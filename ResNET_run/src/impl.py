...
def train_model(num_epochs, train_pairs, valid_pairs, device, model, criterion_type, batch_size,
    ...
    train_loader  = torch.utils.data.DataLoader(train_pairs, shuffle=True, batch_size=batch_size)
    valid_loader  = torch.utils.data.DataLoader(valid_pairs, shuffle=True, batch_size=batch_size)
        ...
        with torch.no_grad():
            for i, (inputs, targets) in enumerate(valid_loader):  
                ...
                if i == 0:  # Adjust according to when and how often you want to plot
                    ...
                    plt.figure()
                    plt.imshow(output_to_plot, cmap='gray')
