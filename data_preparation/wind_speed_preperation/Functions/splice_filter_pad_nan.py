...
def splice_filter_pad_asc(file,angle,cutoff_percentage):
    ...
    dataset_separated = np.nan_to_num(dataset_separated, copy=True, nan=0.0, posinf=None, neginf=None)
    ...
    metadata.to_csv('./Output/Metadata/metadata_asc_nan.csv', index=False)
...
def splice_filter_pad_des(file,angle,cutoff_percentage):
    ...
    #Convert metadata list to dataframe for export
    metadata = pd.DataFrame(metadata_entries)
    metadata.to_csv('./Output/Metadata/metadata_des_nan.csv', index=False)
